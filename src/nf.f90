...
!       integer(4), len :: cap   !capacity of the bin
      integer(4) :: n, n_list(30)  !id of the bin
  end type

  INTEGER(4), allocatable, target :: neighbor(:,:), n_neighbor(:)
...
SUBROUTINE  neighbor_finder
  ...
  integer(4) :: bincap = 10 !need large bincap when performing rdf analysis.
  ...
  type(bins), allocatable :: cellbins(:,:,:)
  ...
  allocate(neighbor(natom, 10), STAT=ierr, ERRMSG=emsg)
  allocate(n_neighbor(natom), STAT=ierr, ERRMSG=emsg)
...
! initialize cell index
  ...
  print *, "CN =  2 3 4 5 6"
  print *, count(n_neighbor==2)," ",count(n_neighbor==3)," ",count(n_neighbor==4),&
  " ",count(n_neighbor==5)," ",count(n_neighbor==6)
